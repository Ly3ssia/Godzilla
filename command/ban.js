...
const configembedembedFooter = "Godzilla" 
  ...
  const xen1 = new MessageEmbed().setDescription("Bu işlemi gerçekleştirebilmek için `BAN_MEMBERS` yetkisine sahip olmalıyım").setColor("RED").setFooter(configembedembedFooter).setAuthor("Godzilla").setTimestamp()
  ...
  const xen4 = new MessageEmbed().setDescription(`${message.author} Banlamak istediğin kişi senden üst yada aynı pozisyonda.`).setColor("RANDOM").setFooter(configembedembedFooter).setAuthor("Godzilla").setTimestamp()
  const xen5 = new MessageEmbed().setDescription(`${message.author} sunucu sahibini banlayamazsın`).setColor("RANDOM").setFooter(configembedembedFooter).setAuthor("Godzilla").setTimestamp()
